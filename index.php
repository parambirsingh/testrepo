...
?>
Core Xii's frequent word sequence finders to get number of millions<br />
...
if (isset($_GET['file']) && $_GET['file'] != '') {
    ...
    if (!$file = @file_get_contents($_GET['file'] . '.txt'))
        die('Nonexistant or empty file.');
    if (isset($_GET['minfreq'])) {
        if (!@preg_match("(\A[1-9][0-9]*\Z)", $_GET['minfreq']))
            die('Invalid minimum frequency. Must be an integer >= 1.');
        else
            $minfreq = (int) $_GET['minfreq'];
    } else
        $minfreq = 3;
    if ($_GET['minchars']) {
        if (!@preg_match("(\A[1-9][0-9]*\Z)", $_GET['minchars']))
            die('Invalid minimum character count. Must be an integer >= 1.');
        else
            $minchars = (int) $_GET['minchars'];
    } else
        $minchars = 3;
    if (isset($_GET['minseq']) && $_GET['minseq'] > 0) {
        if (!@preg_match("(\A[1-9][0-9]*\Z)", $_GET['minseq']))
            die('Invalid minimum sequence length. Must be an integer >= 1.');
        else
            $minseq = (int) $_GET['minseq'];
    } else
        $minseq = 1;
    if ($_GET['maxseq']) {
        if (!@preg_match("(\A[1-9][0-9]*\Z)", $_GET['maxseq']))
            die('Invalid maximum sequence length. Must be an integer >= 1.');
        else if ((int) $_GET['maxseq'] <= $minseq)
            die('Invalid maximum sequence length. Must be more than minimum!');
        else
            $maxseq = (int) $_GET['maxseq'];
    } else{
        $maxseq = 3;
    }
    ...
    $dir = opendir('/home/sg315/public_html/corexii/freqwordseq/');
    while (($f = readdir($dir)) !== false) {
        if (basename($f) == '.' || basename($f) == '..' || substr(basename($f), -4) != '.txt' || basename($f) == 'common_words.txt')
            continue;
        ?><li><?= substr(basename($f), 0, -4) ?></li><?php
        }
        closedir($dir);
